...
use crate::{
    state::{
        self, BPipe, Exception, GlobalIdManager, Id, IdAllocator, Message, MessageData,
        MessageMetaData, State, Throw, Throws, stream_read_bytes_blocking, stream_try_read_bytes,
        stream_write_bytes,
    },
    try_catch,
};
...
impl MessagePipe {
    pub fn from_stream(stream: TcpStream) -> Self {
        Self {
            write: MessageWriter::Stream { s: stream },
        }
    }
    pub fn from_pipe(pipe: BPipe<Message>) -> Self {
        Self {
            write: MessageWriter::Pipe { p: pipe },
        }
    }
    ...
    pub fn try_read_message(&mut self) -> Throws<Option<Message>> {
        match &mut self.write {
            MessageWriter::Pipe { p } => p.recieve(),
            MessageWriter::Stream { s } => {
    ...
    pub fn read_message_blocking(&mut self) -> Throws<Message> {
        match &mut self.write {
            MessageWriter::Pipe { p } => p.recieve_wait(),
            MessageWriter::Stream { s } => {
...
pub enum ServerCtl {
    Kill,
    LocalConnection { con: BPipe<Message> },
    SaveState { to: String },
}

pub struct ClientState {
    pub pipe: MessagePipe,
    pub id: Id,
    pub start_page: u64,
    pub username: String,
}
    ...
    pub fn run(&mut self) {
        ...
        loop {
            ...
            for i in messages {
                match &i.data {
                    MessageData::Connect {
                        ...
                        println!("{} connected", username.username);
                        if let Some(s) = self.clients.get_mut(connected_id) {
                            s.username = username.username.clone();
                        }
                        updates.push(i);
                    }
                    ...
                    MessageData::ImageUpload { name: _, data: _ } => {
                        updates.push(i.clone());
                    }
                    _ => {
                        updates.push(i.clone());
                        let r = self.this_state.handle_message(i);
                        if r.is_err() {
            ...
            for i in state_update_requests {
                let Some(state) = self.clients.get_mut(&i) else {
                    continue;
                };
                try_catch!(try {
                    state.pipe.write_message(Message { meta: MessageMetaData { sender: self.self_id }, data:  MessageData::FullStateUpdate { state: self.this_state.clone(), connections:connections.clone()}})?;
                } catch(_e){
                    dropped_connections.push(i);
                });
            }
            for i in updates {
                for (id, j) in &mut self.clients {
                    if i.meta.sender == *id {
                        continue;
                    }
                    if let Err(_) = j.pipe.write_message(i.clone()) {
                        dropped_connections.push(*id);
                    }
                }
            }
            for i in dropped_connections {
                let Some(cl) = self.clients.get(&i) else {
                    continue;
                };
                self.global.free_bloc(cl.start_page);
                self.clients.remove(&i);
            }
            for mut i in new_connections {
                let id = self.allocator.alloc();
                let start = self.global.alloc_bloc();
                let e = i.write_message(Message {
                    meta: MessageMetaData {
                        sender: self.self_id,
                    },
                    data: MessageData::HandShake {
                        state: self.this_state.clone(),
                        allocator_start: start,
                        images: self.images.clone(),
                        users: connections.clone(),
                        user_id: id,
                    },
                });
                if e.is_err() {
                    self.allocator.dealloc(id);
                    self.global.free_bloc(start);
                    continue;
                }
                self.clients.insert(
                    id,
