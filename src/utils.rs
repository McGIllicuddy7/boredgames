...
pub fn read_object<'a,T:Deserialize<'a>>(stream:&mut TcpStream, buffer:&'a mut  Vec<u8>)->throws!(T){
    let mut buff = [0; 8];
...
pub fn try_read_object<'a,T:Deserialize<'a>>(stream:&mut TcpStream, buffer:&'a mut  Vec<u8>)->throws!(Option<T>){
